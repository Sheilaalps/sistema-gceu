...
import { Eye, EyeOff, Copy, Check, ArrowLeft, BarChart3, Mail, Home, Settings } from 'lucide-react';

// Importando o componente de relatório (verifique se o caminho está correto)
...
const Admin = () => {
    ...
    const buscarRelatorio = async () => {
        setLoading(true);
        try {
            const { data, error } = await supabase.from('usuarios').select('nivel, created_at');
            if (error) throw error;
            
            // Formata os dados para o componente PainelRelatorio entender a data
    ...
    return (
        <div className="admin-container" style={{ background: 'linear-gradient(135deg, #0f0c29 0%, #302b63 50%, #24243e 100%)', minHeight: '100vh' }}>
            ...
            {/* RELATÓRIO DINÂMICO */}
            {exibirRelatorio && <PainelRelatorio dadosIniciais={dadosRelatorio} />}
                ...
                <div className="cadastro-usuarios-section card-glass-effect">
                    ...
                    <form onSubmit={handleCadastro} className="form-cadastro-admin">
                        ...
                        <div className="senha-group">
                            <div className="senha-input-container">
                                <div className="input-with-actions">
                                    <input 
                                        ...
                                        placeholder="Senha" 
                                        value={form.senha} 
                                        onChange={e => {setForm({...form, senha: e.target.value}); checarForcaSenha(e.target.value)}} 
                                    />
                                    <div className="icon-actions-wrapper">
                                        <button type="button" className="btn-input-icon" onClick={() => setMostrarSenha(!mostrarSenha)}>
                                            {mostrarSenha ? <EyeOff size={18} /> : <Eye size={18} />}
                                        </button>
                                        {form.senha && (
                                            <button type="button" className="btn-input-icon" onClick={() => {navigator.clipboard.writeText(form.senha); setCopiado(true); setTimeout(() => setCopiado(false), 2000)}}>
                                                {copiado ? <Check size={18} color="#28a745" /> : <Copy size={18} />}
                                            </button>
                                        )}
                                    </div>
                                </div>
                            ...
                            {/* Aviso de cópia corrigido */}
                            {avisoGerada && <span className="aviso-senha-temp" style={{ color: '#667eea', fontSize: '13px', display: 'block', margin: '5px 0' }}>✨ Senha copiada com sucesso!</span>}
